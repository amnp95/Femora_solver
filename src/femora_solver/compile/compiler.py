...
class Compiler:
    def full_compile(self, model: "Model") -> ExecutionPlan:
        # 1. Build Fields - detect from blocks
        ...
        # Build a compact time-function table (arrays only; no Python callables).
        # time_fn spec supports:
        #   - None            -> Constant(scale=1)
        #   - float/int       -> Constant(scale=value)
        #   - dict            -> {kind: 'Constant'|'Ricker', ...}
        ...
        def _normalize_time_fn(spec):
            ...
            if isinstance(spec, dict):
                ...
                if kind in ("ricker", "ricker_wavelet", "rickerwavelet"):
                    # Ricker: amp * (1 - 2a) * exp(-a), a = (pi*f0*(t-t0))^2
            ...
            if hasattr(block, "compute_lumped_mass"):
                ...
                for f_name, (indices, values) in block_mass.items():
                    if f_name in mass_arrays:
                        ...
                        # values is now expected to be (n_elem, n_nodes_per_elem, ncomp)
